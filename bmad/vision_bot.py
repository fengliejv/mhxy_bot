...
from .ocr.local import detect_coord, flatten_ocr_text, run_local_ocr
...
def _enhance_color_text_bgr(
    img_bgr: np.ndarray,
    mask: np.ndarray,
    *,
    background_scale: float = 0.18,
    alpha: float = 1.35,
    beta: float = 8.0,
) -> np.ndarray:
    gray = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2GRAY)
    base = np.clip(gray.astype(np.float32) * float(background_scale), 0, 255).astype(np.uint8)
    base_bgr = cv2.cvtColor(base, cv2.COLOR_GRAY2BGR)
    boosted = cv2.convertScaleAbs(img_bgr, alpha=float(alpha), beta=float(beta))
    mask3 = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR)
    return np.where(mask3 > 0, boosted, base_bgr)
...
def _build_text_ocr_variant_map(img_bgr: np.ndarray) -> Dict[str, np.ndarray]:
    ...
    white_mask = cv2.inRange(hsv, (0, 0, 185), (180, 70, 255))
    yellow_core_mask = cv2.inRange(hsv, (12, 45, 115), (40, 255, 255))
    yellow_bright_mask = cv2.inRange(hsv, (10, 20, 150), (45, 170, 255))
    yellow_mask = cv2.bitwise_or(yellow_core_mask, yellow_bright_mask)
    blue_purple_mask = cv2.inRange(hsv, (105, 80, 110), (155, 255, 255))

    return {
        "original": img_bgr,
        "gray_bin": _mask_to_ocr_bgr(gray_bin, close_iterations=1, dilate_iterations=1),
        "yellow_text": _enhance_color_text_bgr(
            img_bgr,
            _mask_to_ocr_bgr(yellow_mask, close_iterations=1, dilate_iterations=1)[:, :, 0],
            background_scale=0.14,
            alpha=1.45,
            beta=10.0,
        ),
        "blue_purple_text": _mask_to_ocr_bgr(
            cv2.bitwise_or(blue_purple_mask, white_mask), open_iterations=1, close_iterations=1
        ),
    }
...
def _crop_center_region(
    ...
    return img_bgr[top:bottom, left:right].copy(), (left, top)
...
def _collect_text_ocr_candidates_from_variants(
    ...
    for variant_name in selected_variant_names:
        ...
        resp = run_local_ocr(
            ...
            use_cls=False,
            use_rec=True,
            log_prefix=f"{log_prefix}[{variant_name}]",
        )
...
def find_text_by_local_ocr(
    image: Any,
    keyword: str,
    variant_names: Optional[Sequence[str]] = None,
    center_crop_ratio: Optional[float] = None,
) -> Optional[Dict[str, Any]]:
    debug_img = image_io.to_bgr(image)
    search_img = debug_img
    crop_offset = (0, 0)
    if center_crop_ratio is not None:
        search_img, crop_offset = _crop_center_region(debug_img, float(center_crop_ratio))
    variant_names = tuple(variant_names or ("original", "gray_bin"))
    candidates = list(
        _collect_text_ocr_candidates_from_variants(search_img, log_prefix="[vision_bot]", variant_names=variant_names)
    )
    if crop_offset != (0, 0):
        offset_x, offset_y = crop_offset
        for item in candidates:
            item["center"] = (int(item["center"][0]) + offset_x, int(item["center"][1]) + offset_y)
            item["box"] = [(int(x) + offset_x, int(y) + offset_y) for x, y in item["box"]]

    print(
        f"[vision_bot] local_text_ocr keyword={keyword!r} variants={list(variant_names)!r} texts={[item['text'] for item in candidates]}"
    )
    best = _pick_best_text_match(candidates, keyword)
    if best is None:
...
def _input_with_number_keyboard(value: int) -> Dict[str, Any]:
    text = str(int(value))
    if not text.isdigit():
...
def navigate_to_coord(x: int, y: int) -> Dict[str, Any]:
    ...
    print(
        ...
        f"tap_map_button={tap_map_button} tap_x={tap_x} input_x={input_x} tap_y=auto_focus input_y={input_y} tap_go={tap_go} "
...
def _crop_png_bytes(img_bgr: np.ndarray, roi: Tuple[int, int, int, int]) -> bytes:
    ...
    return image_io.to_png_bytes(cropped)
