...
from .vision.matcher import match_template
from .device import operator as operator_util
from .vision import bot as vision_bot

_SHEYAOXIANG_TPL_TISHI = "assets/android/changjing/sheyaoxiangtishi.png"
_SHEYAOXIANG_TPL_SHIYONG = "assets/android/changjing/shiyong.png"
_SHEYAOXIANG_CHECK_MAPS = {"大唐国境", "江南野外", "长寿郊外", "东海湾", "花果山"}


def _maybe_use_sheyaoxiang(map_name: str) -> Dict[str, Any]:
    img_bgr = operator_util.screenshot_bgr()
    has_prompt = operator_util.template_exists(
        _SHEYAOXIANG_TPL_TISHI,
        threshold=botconfig.ANDROID_MATCH_THRESHOLD,
        img_bgr=img_bgr,
    )
    if not has_prompt:
        return {"checked": True, "map_name": map_name, "has_prompt": False, "used": False}

    tapped = operator_util.try_tap_with_retry(
        _SHEYAOXIANG_TPL_SHIYONG,
        threshold=botconfig.ANDROID_MATCH_THRESHOLD,
        retry=3,
        retry_sleep_s=botconfig.ANDROID_STEP_SLEEP_S,
    )
    used = tapped is not None
    print(f"[sheyaoxiang] map={map_name} has_prompt=True used={used} tap={tapped}")
    return {"checked": True, "map_name": map_name, "has_prompt": True, "used": used, "tap": tapped}
...
def route_to_datangguojing(coord: Tuple[int, int]) -> Dict[str, Any]:
    ...
    tap_hide_ui = (
        vision_bot.try_tap(
            botconfig.ANDROID_TPL_SYSTEM_HIDE_UI_DISABLE, threshold=botconfig.ANDROID_THR_SYSTEM_HIDE_UI_DISABLE
        )
...
def route_by_map(map_name: str, coord: Optional[Tuple[int, int]]) -> Dict[str, Any]:
    ...
    if dest in route_handlers:
        result = _route_with_required_coord(map_name, coord, route_handlers[dest])
        if bool(result.get("ok")) and dest in _SHEYAOXIANG_CHECK_MAPS:
            result["sheyaoxiang"] = _maybe_use_sheyaoxiang(dest)
        return result
