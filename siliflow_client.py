...
def siliconflow_chat_completions(  # 调用 SiliconFlow 的 /chat/completions 接口
    ...
    api_key = botconfig.env_str("SILICONFLOW_API_KEY", "")  # 读取/覆盖 API Key
    if not api_key:  # 没有拿到有效 key
        raise RuntimeError("缺少 SILICONFLOW_API_KEY，请在环境变量或 .env 中配置")  # 直接报错

    base_url = botconfig.env_str("SILICONFLOW_BASE_URL", "https://api.siliconflow.cn/v1").rstrip("/")  # 读取/覆盖 base_url
...
def siliconflow_paddleocr(  # 用 PaddleOCR-VL 作为多模态 OCR：输入图片，输出文本
    ...
    model = botconfig.env_str("SILICONFLOW_OCR_MODEL", "PaddlePaddle/PaddleOCR-VL-1.5")  # 选择 OCR 模型
...
def siliconflow_qwen_structured(  # 用 Qwen-VL 做结构化输出：输入图片 + 提示词，输出 JSON
    ...
    model = botconfig.env_str("SILICONFLOW_QWEN_MODEL", "Qwen/Qwen3.6-VL-Plus")  # 选择 VL 模型
