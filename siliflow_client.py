...
def siliconflow_chat_completions(  # 调用 SiliconFlow 的 /chat/completions 接口
    ...
    api_key = botconfig.SILICONFLOW_API_KEY  # 读取/覆盖 API Key
    if not api_key:  # 没有拿到有效 key
        raise RuntimeError(f"缺少 {botconfig.KEY_SILICONFLOW_API_KEY}，请在环境变量或 .env 中配置")  # 直接报错

    base_url = botconfig.SILICONFLOW_BASE_URL.rstrip("/")  # 读取/覆盖 base_url
...
def siliconflow_paddleocr(  # 用 PaddleOCR-VL 作为多模态 OCR：输入图片，输出文本
    ...
    model = botconfig.SILICONFLOW_OCR_MODEL  # 选择 OCR 模型
...
def siliconflow_qwen_structured(  # 用 Qwen-VL 做结构化输出：输入图片 + 提示词，输出 JSON
    ...
    model = botconfig.SILICONFLOW_QWEN_MODEL  # 选择 VL 模型
